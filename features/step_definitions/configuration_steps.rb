...
Then(/^AI_MODEL should be set from the config file$/) do
  expect(@loaded_config['AI_MODEL']).to eq('qwen3.5-9b-unsloth:latest')
...
Then(/^nested values should be accessible$/) do
  expect(@loaded_config['PROJECT_CONFIGS']['javascript']['AI_MODEL']).to eq('qwen3.5-9b-unsloth:latest')
...
Then(/^missing variables should use defaults #53$/) do
  ...
  expect(@default_values_applied).to be true
end
