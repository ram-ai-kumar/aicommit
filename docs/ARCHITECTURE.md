...
| `lib/core.sh` | 7.6 KB | Core logic — prerequisite validation, context building, prompt assembly, Ollama integration |
...
| `generate_commit_message()` | Uses `awk` to substitute placeholders in prompt template, pipes to `ollama run`, extracts message via `@@@` delimiters |
